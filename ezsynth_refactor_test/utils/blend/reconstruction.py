...
class reconstructor():
    ...
    Class should use '__iter__', '__next__' and '__call__' methods.
    Example
    -------
    ...
    def _create(self):
        for i in range(len(self.hist_blends)):
            blend = poisson_fusion(self.hist_blends[i], self.style_fwd[i], self.style_bwd[i], self.err_masks[i])
