...
def create_guides(a: Config) -> Guides:
    ...
    flow_rev = list(optical_flow_processor(a.frames))
    flow_fwd = [x * -1 for x in flow_rev]

    print("Analyzing position.")
    positional_rev = PositionalGuide(a.frames, flow = flow_rev)()
    positional_fwd = PositionalGuide(a.frames, flow = flow_rev[::-1])()
