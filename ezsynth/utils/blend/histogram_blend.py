...
class HistogramBlender:
    ...
    def assemble_min_error_img(self, a: np.ndarray, b: np.ndarray, error_mask):
    ...
    def blend(
        ...
        with threading.Lock():
            ...
            ab = self.histogram_transform(
                ab, ab_mean, ab_std, min_error_mean, min_error_std
            )
            ab = np.round(ab)
            ab = np.clip(ab, 0, 255)
